const { xiCiProxyListUrl } = require('../config');
...
const getProxyPage = async(page) => {
    try {
		const pageUrl = `${xiCiProxyListUrl}/${page}`
...
const echoProxy = async() => {
	...
	await Promise.fromCallback((cb) => {
		fs.writeFile(`${__dirname}/xiciOkProxy.json`, JSON.stringify(allProxyArr), cb);
	});
